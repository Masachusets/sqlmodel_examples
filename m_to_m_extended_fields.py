...
def create_teams_and_heroes():
    with Session(engine) as session:
        ...
        deadpond_team_z_link = HeroTeamLink(team=team_z_force,
                                            hero=hero_deadpond)
        deadpond_preventers_link = HeroTeamLink(team=team_preventers,
                                                hero=hero_deadpond,
                                                is_training=True)
        spider_boy_preventers_link = HeroTeamLink(team=team_preventers,
                                                  hero=hero_spider_boy,
                                                  is_training=True)
        rusty_man_preventers_link = HeroTeamLink(team=team_preventers,
                                                hero=hero_rusty_man)
...
def update_teams():
    with Session(engine) as session:
        ...
        spider_boy_z_force_link = HeroTeamLink(team=team_z_force,
                                               hero=hero_spider_boy,
                                               is_training=True)
...
def update_heroes():
    with Session(engine) as session:
        ...
        for link in hero_spider_boy.team_links:
            print("Spider-Boy's teams:", link.team, '\n',
                  "is training:", link.is_training)
